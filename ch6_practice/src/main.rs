use graphics::math::{add, mul, mul_scalar, Vec2d};
use piston_window::*;
use rand::prelude::*;
...
/// 这里其实就是给原生的allocator加一个report的皮。。
...
impl Particle {
    fn new(world: &World) -> Particle {
        ...
        let y = world.height;
        let x_v = 0.0;
        let y_v = rng.gen_range(-2.0..0.0);
        let x_a = 0.0;
        let y_a = rng.gen_range(0.0..0.15);

        Particle {
            height: 4.0,
            width: 4.0,
            position: [x, y].into(),
            velocity: [x_v, y_v].into(),
            acceleration: [x_a, y_a].into(),
...
impl World {
    ...
    fn add_shapes(&mut self, n: i32) {
        for _ in 0..n.abs() {
            // particle 是 stack allocated。因为 Particle 的大小编译器提前知道
